...
apiRoutes.post('/blogs', function(req, res) {
    var nick = new Blogs({
        title: '我的测试数据',
        eg: '备注信息',
        content: '文章的主体内容，文章的主体内容文章的主体内容文章的主体内容文章的主体内容，文章的主体内容文章的主体内容文章的主体内容；',
        author: '殖民123',
        data: new Date(),
    });
...
apiRoutes.get('/users', function(req, res) {
    ...
    // User.findByName('xsk123',function(err,users){
    //     res.json(users);
    // })
...
//批量处理传递数组形式，根据_id批量查询
apiRoutes.get('/usersQuerys', function(req, res) {
    var findObj = req.query;
    User.find(findObj, function(err, users) {
        if (err) {
            res.json(err);
        }
        res.json(users);
